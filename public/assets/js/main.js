...
(function () {
    ...
    let styleSwitcherToggleEl = document.querySelector(
    ...
    if (window.templateCustomizer) {
        ...
        if (window.Helpers.isLightStyle()) {
            if (styleSwitcherToggleEl) {
                styleSwitcherToggleEl
                    .querySelector("i")
                    .classList.add("bx-moon");
            ...
            for (let i = 0; i < dropdownItems.length; i++) {
                dropdownItems[i].addEventListener("click", function () {
                    let currentLanguage = this.getAttribute("data-language"),
                        selectedLangFlag =
                            this.querySelector(".fi").getAttribute("class"),
                        startsWith = "fs-",
                        classes = selectedLangFlag
                            .split(" ")
                            .filter(function (v) {
                    ...
                    languageDropdown[0].querySelector(
                        ".dropdown-toggle .fi"
                    ).className = selectedLangFlag;

                    i18next.changeLanguage(currentLanguage, (err, t) => {
                        if (err)
                            return console.log(
                                "ایرادی در بارگذاری پیش آمد",
                                err
                            );
                        localize();
                    });
